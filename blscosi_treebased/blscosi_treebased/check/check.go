...
import (
	...
	"github.com/dedis/student_19_gossip_bls/blscosi_treebased"
...
func SignStatement(msg []byte, ro *onet.Roster) (*blscosi_treebased.SignatureResponse, error) {
	client := blscosi_treebased.NewClient()
	publics := ro.ServicePublics(blscosi_treebased.ServiceName)

	log.Lvlf4("Signing message %x", msg)

	pchan := make(chan *blscosi_treebased.SignatureResponse, 1)
...
func VerifySignatureHash(b []byte, sig *blscosi_treebased.SignatureResponse, ro *onet.Roster) error {
	suite := blscosi_treebased.NewClient().Suite().(*pairing.SuiteBn256)
	publics := ro.ServicePublics(blscosi_treebased.ServiceName)
